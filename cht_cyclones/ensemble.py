"""
Tropical-cyclone ensemble generation.

Provides :class:`TropicalCycloneEnsemble` and :class:`TropicalCycloneEnsembleMember`,
which implement the probabilistic track-perturbation approach based on
DeMaria et al. (2009).  Each member's track is perturbed by drawing random
along-track, cross-track, and intensity errors from auto-regressive processes
calibrated on NHC statistics.
"""
...
class TropicalCycloneEnsemble:
    """
    Collection of perturbed tropical-cyclone tracks and optional wind fields.

    The first member (index 0) is always the unperturbed best track.
    Subsequent members are generated by :meth:`generate`.

    Parameters
    ----------
    tropical_cyclone : TropicalCyclone
        The deterministic best-track object to perturb.
    name : str, optional
        Base name used when writing track and spiderweb files.
    number_of_realizations : int, optional
        Number of perturbed members (excluding the best track).
    compute_wind_fields : bool, optional
        If ``True``, compute scaled spiderweb wind fields for every member.
    dt : float, optional
        Time step (hours) used for resampling the track before perturbation.
    tstart : datetime, optional
        Start of the forecast period; perturbations are applied after this time.
    tend : datetime, optional
        End time to which the track is shortened.
    duration : float, optional
        Alternative to ``tend``; duration in hours from ``tstart``.
    track_path : str, optional
        Output directory for track files.
    spw_path : str, optional
        Output directory for spiderweb files.
    mean_abs_cte24 : float, optional
        Mean absolute 24-hour cross-track error (nautical miles).
    sc_cte : float, optional
        Auto-regression coefficient for the cross-track error.
    mean_abs_ate24 : float, optional
        Mean absolute 24-hour along-track error (nautical miles).
    sc_ate : float, optional
        Auto-regression coefficient for the along-track error.
    mean_abs_ve24 : float, optional
        Mean absolute 24-hour intensity error (knots).
    sc_ve : float, optional
        Auto-regression coefficient for the intensity error.
    bias_ve : float, optional
        Intensity bias per forecast hour.
    format : str, optional
        Output format for spiderweb files: ``"ascii"`` or ``"netcdf"``.
    """
    ...
    def generate_tracks(self) -> None:
        """Resample the best track and generate perturbed tracks for all members."""
        ...
        for i in range(0, self.number_of_realizations):
            print(
                f"Generating ensemble track {i + 1} of {self.number_of_realizations} ..."
            )
    ...
    def make_wind_fields(self, option: str = "scaling") -> None:
        """
        Compute spiderweb wind fields for all ensemble members.

        Parameters
        ----------
        option : str, optional
            Method to derive member wind fields.  Currently only ``"scaling"``
            is supported, which scales the best-track spiderweb by the per-time-step
            wind-scale factor of each member.
        """
        for i in range(self.number_of_realizations):
            print(
                f"Generating ensemble wind field {i + 1} of {self.number_of_realizations} ..."
            )
...
class TropicalCycloneEnsembleMember:
    """A single member of a :class:`TropicalCycloneEnsemble`."""
    ...
    def generate_random_track(
        self,
        tstart_ensemble: datetime,
        mean_abs_ate24: float,
        sc_ate: float,
        mean_abs_cte24: float,
        sc_cte: float,
        mean_abs_ve24: float,
        sc_ve: float,
        bias_ve: float,
        ...
        ----------
        tstart_ensemble : datetime
            Start of the forecast window; perturbations are zero before this.
        mean_abs_ate24 : float
            Mean absolute 24-hour along-track error (m).
        sc_ate : float
            Auto-regression scaling factor for ATE.
        mean_abs_cte24 : float
            Mean absolute 24-hour cross-track error (m).
        sc_cte : float
            Auto-regression scaling factor for CTE.
        mean_abs_ve24 : float
            Mean absolute 24-hour intensity error (same unit as track vmax).
        sc_ve : float
