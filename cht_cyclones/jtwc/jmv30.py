"""
Reader for JMV 3.0 tropical-cyclone track files (``.tcw``).

JMV 3.0 is the format used by the Joint Typhoon Warning Center (JTWC) for
issuing tropical-cyclone advisories.  The file contains a forecast section
(tau-based track points) and an optional hindcast section, which are merged
...
def datenum_to_datetime(dn: float) -> datetime:
    """
    Convert a MATLAB datenum to a Python :class:`datetime`.
...
def datetime_to_datenum(dt: datetime) -> float:
    """
    Convert a Python :class:`datetime` to a MATLAB datenum.
    ...
    -------
    float
        Equivalent MATLAB serial date number.
    """
    return (
        dt.toordinal()
        + 366
...
def jmv30_to_dict(fname: str) -> dict:
    """
    Parse a JMV 3.0 ``.tcw`` file and return the contents as a dictionary.

    Both the forecast (tau-based) and hindcast sections are parsed and merged.

    Parameters
    ----------
    fname : str
        Path to the ``.tcw`` advisory file.

    Returns
    -------
    dict
        Dictionary with keys ``"name"``, ``"advisorynumber"``, ``"time"``,
        ``"x"``, ``"y"``, ``"vmax"``, ``"r35ne"``/``"r35se"``/... etc., and
        ``"first_forecast_time"``.
    """
    tc = {
        "name": "",
...
def to_gdf(fname: str) -> tuple:
    """
    Read a JMV 3.0 file and return a GeoDataFrame of the track.
    ...
    gdf : geopandas.GeoDataFrame
        ...
        ``vmax``, ``pc``, ``rmw``, and quadrant wind radii.
    name : str
        Storm name extracted from the file.
    advisory : int or None
        Advisory number extracted from the file.
    """
